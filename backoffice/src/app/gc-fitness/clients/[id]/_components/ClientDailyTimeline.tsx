...
import { CalendarDays, Camera, CheckCircle2, MessageSquare, NotebookText, Dumbbell, Clock3, Plus } from "lucide-react";

import type { ClientDailyTimeline as Timeline } from "@/lib/gc-fitness/client-daily-timeline-actions";
...
export function ClientDailyTimeline({
  ...
  const initialDate = timeline.days.find((day) => day.date === today)?.date ?? timeline.days[0]?.date ?? today;
  const [selectedDate, setSelectedDate] = useState(initialDate);
  const selected = useMemo(
    () => timeline.days.find((day) => day.date === selectedDate) ?? timeline.days[0],
    [selectedDate, timeline.days],
  );
      ...
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Workouts, habits, photos, notes, and chat context by day.
          </p>
        </div>
      </div>
      ...
      <div className="mb-4 flex gap-2 overflow-x-auto pb-1">
        {timeline.days.map((day) => {
          ...
          return (
            ...
            >
              <span className="block font-medium">{shortDate(day.date)}</span>
              <span className={day.date === selectedDate ? "text-primary-foreground/75" : "text-muted-foreground"}>
                {hasData ? "Activity" : "No data"}
              </span>
            </button>
          );
        })}
                ...
                <li key={photo.id} className="rounded-md bg-muted px-3 py-3">
                  <div className="flex items-start gap-3">
                    <div className="relative h-16 w-12 shrink-0 overflow-hidden rounded-md bg-background">
                      {photo.url ? (
                        <Image src={photo.url} alt={photo.caption || "Progress photo"} fill className="object-cover" sizes="48px" />
                      ) : (
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {photo.caption ? <p className="line-clamp-2 text-xs text-muted-foreground">{photo.caption}</p> : null}
                      <Link href="#progress-photos" className="mt-1 inline-flex items-center gap-1 text-xs text-primary hover:underline">
                        View side-by-side
                      </Link>
                    </div>
                  </div>
